...
function promptForBatch(
  ...
  return [
    ...
    "- If a question number has ONLY labelled sub-parts ((i), (ii), (a), (b), etc.) and no standalone content beyond a shared instruction/stem line, do NOT emit a separate top-level entry for the bare number. Emit only the sub-part entries. Fold the shared stem into each sub-part's `text` so each entry is self-contained. Do still emit a bare-number entry when that number has its own standalone question text, not just a stem.",
    "- Worked example of that stem-only pattern. Printed:",
    "  11. Design a sequential circuit to detect the overlapping sequence 101:",
    "      (i) draw the state diagram",
    "      (ii) implement it using D flip-flops",
    "  Correct — two entries only, no bare \"11\":",
    '  [{"page":0,"number":"11","subpart":"(i)","text":"Design a sequential circuit to detect the overlapping sequence 101: draw the state diagram"},{"page":0,"number":"11","subpart":"(ii)","text":"Design a sequential circuit to detect the overlapping sequence 101: implement it using D flip-flops"}]',
    "  Incorrect — do not also emit {\"number\":\"11\",\"subpart\":null,\"text\":\"Design a sequential circuit to detect the overlapping sequence 101:\"}.",
